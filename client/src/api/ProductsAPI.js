...
function ProductsAPI() {
    ...
    useEffect(() => {
        ...
        const res = await axios.get(`/api/products?limit=${page * 10}&&${category ? `category=${category}&` : ''}${subcategory ? `subcategory=${subcategory}&` : ''}&${sort}&title[regex]=${search}&condition=${condition}`);
